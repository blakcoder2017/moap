...
var sql =
"CREATE TABLE IF NOT EXISTS ben (id INTEGER PRIMARY KEY AUTOINCREMENT, benid text NOT NULL,fname text NOT NULL,surname text NOT NULL,gender text NOT NULL,yob text NOT NULL,group_name text NOT NULL,prev_ben text NOT NULL,hhsize text NOT NULL,gps text NOT NULL,value_chain text NOT NULL,community text NOT NULL,region text NOT NULL,district text NOT NULL,farm_size text NOT NULL,role text NOT NULL,pic text NOT NULL,officer text NOT NULL,regdate text NOT NULL)";
  ...
  var sql =
      "CREATE TABLE IF NOT EXISTS benOld (id INTEGER PRIMARY KEY AUTOINCREMENT, benid text NOT NULL,fname text NOT NULL,surname text NOT NULL,gender text NOT NULL,yob text NOT NULL,group_name text NOT NULL,prev_ben text NOT NULL,hhsize text NOT NULL,gps text NOT NULL,value_chain text NOT NULL,community text NOT NULL,region text NOT NULL,district text NOT NULL,farm_size text NOT NULL,role text NOT NULL,pic text NOT NULL,officer text NOT NULL,regdate text NOT NULL)";
  db.transaction(function (transaction) {
      transaction.executeSql(sql, [], showRecords, handleErrors);
      console.debug("executeSql: " + sql);
  });
...
function insertRecord() {
  ...
  var sql =
    "INSERT INTO ben (benid,fname,surname,gender,yob,group_name,prev_ben,hhsize,gps,value_chain,community,region,district,farm_size,role,pic,officer,regdate) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
...
function updateRecord() {
  ...
  var sql =
    "UPDATE ben SET fname=?,surname=?,gender=?,yob=?,group_name=?,prev_ben=?,hhsize=?,gps=?,value_chain=?,community=?,region=?,district=?,farm_size=?,role=?,officer=? WHERE id=?";
...
function renderRecords(transaction, results) {
  ...
  if (dataset.length > 0) {
    ...
    for (var i = 0, item = null; i < dataset.length; i++) {
      ...
      html =
        html +
        '        <button type="button" data-icon="arrow-u" onClick="prepareEdit(' +
        i +
        ');">edit</button>';
      html =
        html +
        '        <button type"button" data-icon="delete" onClick="deleteRecord(' +
        item["id"] +
        ');">delete</button>';
      html = html + "      </p>";
      html = html + "    </li>";
